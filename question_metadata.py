...
BANK_VERSION="2026.08.28-cleanroom.1"
...
def enrich(q,review_status="cleanroom-draft"):
    item=dict(q)
    source_keys=tuple(item.get("source_keys") or DEFAULT_SOURCE_KEYS)
    item.setdefault('objective_id',objective_id(item));item.setdefault('concept_id',item['objective_id']);item.setdefault('item_id',item_id(item))
