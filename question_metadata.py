...
No item is represented as a live FINRA question. Public FINRA materials inform scope/style only.
"""
...
def item_id(q):
    """Stable identity for a genuinely distinct stem/answer pair."""
    raw="|".join(str(q.get(k,"")) for k in ("chapter","term","q","a"))
    return f"{objective_id(q)}-{hashlib.sha1(raw.encode('utf-8')).hexdigest()[:10].upper()}"
...
def option_rationales(q):
    out={};answer=q.get('a');why=q.get('why','');ch=q.get('chapter')
    supplied=q.get("distractor_rationales",{})
    for choice in q.get('c',[]):
        ...
        if d:out[choice]=f"Not best here. {choice} means: {d} The controlling facts point instead to {q.get('term','the tested concept')}."
...
def enrich(q,review_status="draft-reviewed"):
    item=dict(q)
    item.setdefault('objective_id',objective_id(item));item.setdefault('concept_id',item['objective_id'])
    item.setdefault('item_id',item_id(item));item.setdefault('difficulty',infer_difficulty(item));item.setdefault('source_basis',SOURCE_BASIS)
    item.setdefault('bank_version',BANK_VERSION);item.setdefault('last_reviewed',str(date.today()));item.setdefault('review_status',review_status)
