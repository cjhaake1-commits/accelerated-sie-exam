...
def fresh_sample(chapters,n):
    ...
    # Keep enough history to prevent immediate repeats without permanently exhausting the bank.
    st.session_state.question_history=st.session_state.question_history[-400:]
    return q
...
def show_cliffnotes(block):
    chs=BLOCKS[block];st.subheader(f"Block {block}: Chapters {chs[0]}–{chs[-1]}")
    st.caption("CliffNotes-plus: major tested knowledge → reasoning → distinctions → math → terminology. Read for understanding, not just recognition.")
    for ch in chs:
        lesson=LESSONS[ch];deep=DEEP_DIVE[ch];master=BLOCK1_MASTERY.get(ch) if block==1 else None
        with st.expander(f"Chapter {ch}: {CHAPTERS[ch]['title']}",expanded=True):
            st.markdown("### 1. Build the mental model");st.write(deep["narrative"])
            if master:
                st.markdown("### 2. Major topics you actually need to understand")
                for title,text in master["major"]:st.markdown(f"**{title}** — {text}")
                st.markdown("### 3. Reason your way through the question");st.info(master["reason"])
                st.markdown("### 4. Math / calculations / relationships")
                for x in master["math"]:st.markdown(f"- 🧮 {x}")
                st.markdown("### 5. Know these close distinctions")
                st.write(" • ".join(master["distinguish"]))
                offset=6
            else:
                st.markdown("### 2. The core idea");st.info(lesson["big"])
                st.markdown("### 3. Why it works this way")
                for x in lesson["why"]:st.markdown(f"- {x}")
                st.markdown("### 4. How to use it on the exam");st.success(deep["mental"]);offset=5
            st.markdown(f"### {offset}. Why the underlying concepts work")
            for x in lesson["why"]:st.markdown(f"- {x}")
            st.markdown(f"### {offset+1}. Connect it to the rest of the exam");st.write(deep["connection"])
            st.markdown(f"### {offset+2}. Close-answer traps")
            for x in lesson["traps"]:st.markdown(f"- ⚠️ {x}")
            st.markdown(f"### {offset+3}. Question-solving rule");st.success(lesson["apply"])
            st.markdown(f"### {offset+4}. Terms — memorize these LAST")
            for term,definition in CHAPTERS[ch]["cards"]:st.markdown(f"**{term}** — {definition}")

st.markdown('<div class="hero"><div class="big">🎯 Accelerated SIE Exam</div><div class="muted">Understand → Reason → Distinguish → Calculate → Apply → Memorize Terms Last → Test</div></div>',unsafe_allow_html=True)
with st.sidebar:
...
if page=="Dashboard":
    st.subheader("Course flow");st.write("Study blocks teach the major concepts and reasoning. Tests now treat cosmetic rewordings as the same underlying question and suppress recent repeats.")
    for b,chs in BLOCKS.items():st.info(f"Block {b}: Chapters {chs[0]}–{chs[-1]} lessons → 50-question cumulative test")
    ...
    cards=CHAPTERS[ch]["cards"];idx=st.session_state.chapter_card_idx%len(cards);term,definition=cards[idx];st.subheader(f"Chapter {ch}: {CHAPTERS[ch]['title']}");st.caption("Flashcards are deliberately concise. Use Study Blocks for understanding.");st.markdown(f"## {term}")
    ...
    if not st.session_state.block_exam or st.session_state.active_test_block!=block:
        if st.button("Generate 50-Question Test",type="primary",use_container_width=True):build_block(block);st.rerun()
    else:
...
st.divider();st.caption("Original study lessons grounded in the uploaded SIE manual. Public FINRA materials inform exam-skill design; no proprietary/live exam questions are reproduced.")